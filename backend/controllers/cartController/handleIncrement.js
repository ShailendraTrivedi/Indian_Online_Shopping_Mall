...
/** PATCH :  http://localhost:5000/cart/incrementCartItem */
const handleIncrementCartItem = async (req, res) => {
  try {
    ...
    if (cartItem.productQuantity + 1 <= 10) cartItem.productQuantity += 1;
    await userCart.save();

    return res
      .status(200)
      .json({ message: "Item quantity incremented successfully", cartItem });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      error:
...
module.exports = handleIncrementCartItem;
